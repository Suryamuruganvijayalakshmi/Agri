        ...
        if (eventType === 'notification_pushed') {
            ...
            if (targetFarmerId && targetFarmerId !== 'ALL') {
                ...
                await broadcastPushNotification(
                    (payload && payload.title) || 'AGRIFlow Procurement Alert',
                    (payload && payload.message) || 'New operational update received.', { url: (payload && payload.link) || (payload && payload.url) || '/farmer/dashboard' }
                );
            }
        } else if (eventType === 'appointment_booked') {
            const appt = (payload && payload.appointment) || payload;
            const token = (payload && payload.token_number) || (appt && appt.token_number) || 'New Slot';
            const farmer = (appt && appt.farmer_name) || 'Farmer';
            const qty = (appt && appt.declared_quantity_kg) || (appt && appt.quantity_kg) || 2500;
            await broadcastPushNotification(
                `📅 Slot Booked: Token ${token}`,
                `${farmer} booked ${Number(qty).toLocaleString()} kg of ${appt?.crop_type || 'produce'}.`, { url: '/operator/queue' }
            );
        } else if (eventType === 'queue_updated') {
            if ((payload && payload.status === 'CALLED') || (payload && payload.token_number)) {
                await broadcastPushNotification(
                    `📢 Token ${payload.token_number || 'Next'} Called!`,
                    `Proceed to counter / weighbridge immediately.`, { url: '/farmer/queue' }
                );
            }
        } else if (eventType === 'payment_updated') {
            const p = (payload && payload.payment) || payload;
            await broadcastPushNotification(
                `💳 DBT Payment: ${p?.status || 'UPDATED'}`,
                `Payment of ₹${Number(p?.amount || 0).toLocaleString()} for ${p?.farmer_name || 'Farmer'} is ${p?.status}.`, { url: '/farmer/payments' }
            );
        }
    } catch (e) {
...
// ============================================================
// CROP PRODUCTS & MSP RATE MANAGEMENT (Officer Control Panel)
// ============================================================

// GET all crop products with current MSP rates
...
// POST add a new crop product
app.post('/api/products', async(req, res) => {
    try {
        const result = await db.addProduct(req.body);
        if (!result.success) return res.status(400).json(result);
        await broadcastRealtimeUpdate('crop_prices_updated', { product: result.product });
        res.json(result);
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
// POST update a crop MSP rate (Officer action — live price update)
app.post('/api/products/update-price', async(req, res) => {
    try {
        const { product_id, new_price_per_kg, reason, officer_name } = req.body;
        if (!product_id || !new_price_per_kg) {
            return res.status(400).json({ success: false, error: 'product_id and new_price_per_kg are required.' });
        }
        const result = await db.updateProductMSP(product_id, new_price_per_kg);
        if (!result.success) return res.status(400).json(result);

        const products = await db.getAllProducts();
        // Broadcast price change to ALL clients in real time
        io.emit('crop_prices_updated', { products, updated_product_id: product_id, new_price_per_kg, reason: reason || 'MSP rate updated by officer', officer_name: officer_name || 'Officer' });

        console.log(`[AGRIFlow] Officer updated MSP rate for product ${product_id} → ₹${new_price_per_kg}/kg. Reason: ${reason || '-'}`);
        res.json({ success: true, message: `MSP rate updated to ₹${new_price_per_kg}/kg`, products });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
});
...
app.put('/api/farmers/profile', async(req, res) => {
    try {
        ...
        const farmer = await Farmer.findOneAndUpdate({ $or: [{ profile_id: userId }, { phone }] }, {
            $set: {
                village,
                taluk,
                district,
...
app.get('/api/slots', async(req, res) => {
    try {
        ...
        res.json({
            ...
            recommended_slot_id: (recommendedSlot && recommendedSlot.id) || null
        });
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
app.post('/api/appointments/book-position', async(req, res) => {
    try {
        ...
        await broadcastRealtimeUpdate('appointment_booked', {
            ...
            centre_id: result.appointment && result.appointment.centre_id,
        ...
        await broadcastRealtimeUpdate('slot_position_updated', {
            slot_id: result.appointment && result.appointment.slot_id,
            position_number: result.appointment && result.appointment.position_number,
            centre_id: result.appointment && result.appointment.centre_id
        });
...
app.post('/api/appointments/cancel', async(req, res) => {
    try {
        ...
        await broadcastRealtimeUpdate('slots_updated', { centre_id: result.appointment && result.appointment.centre_id });
        res.json(result);
    } catch (error) {
        res.status(500).json({ success: false, error: error.message });
    }
...
app.get('/api/queue/:centreId', async(req, res) => {
    try {
        const farmerId = req.query.farmer_id || (req.user && req.user.userId) || 'default-farmer';
...
app.post('/api/booking/position', async(req, res) => {
    try {
        const result = await db.bookAppointmentPosition(req.body);
        if (!result.success) return res.status(400).json(result);
        await broadcastRealtimeUpdate('appointment_booked', { appointment: result.appointment, centre_id: result.appointment && result.appointment.centre_id });
        await broadcastRealtimeUpdate('queue_updated', { centre_id: result.appointment && result.appointment.centre_id });
        await broadcastRealtimeUpdate('slot_position_updated', { slot_id: result.appointment && result.appointment.slot_id, centre_id: result.appointment && result.appointment.centre_id });
        await broadcastRealtimeUpdate('slots_updated', { centre_id: result.appointment && result.appointment.centre_id });
        ...
        if (result.appointment) {
            ...
            broadcastPushNotification(
                ...
                `Token #${result.appointment.token_number} at ${result.appointment.centre_name}. ${result.appointment.bays_label || 'Bay #' + result.appointment.position_number}`, { url: '/queue' }
            ).catch(() => {});
        }
...
const resolveFarmerId = (req) => {
    return (req.user && req.user.userId) || req.headers['x-farmer-id'] || req.query.farmerId || req.query.farmer_id || null;
};
...
const markAllNotificationsAsRead = async(farmerIdParam, req, res) => {
    try {
        const farmerId = farmerIdParam || resolveFarmerId(req) || (req.body && req.body.farmerId) || (req.body && req.body.farmer_id);
        if (!farmerId) {
            return res.status(400).json({ success: false, error: 'Farmer identity required' });
        }
        await Notification.updateMany({ $or: [{ farmer_id: farmerId }, { farmerId: farmerId }, { farmer_id: 'ALL' }, { farmerId: 'ALL' }], read: false }, { $set: { read: true } });
