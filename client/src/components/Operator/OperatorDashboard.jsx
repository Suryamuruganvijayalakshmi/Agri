...
import { Building2, Users, Weight, ShieldCheck, AlertTriangle, Play, RefreshCw, CheckCircle2, ChevronRight, Sliders, Edit3, RotateCcw } from 'lucide-react';
import { updateOperatorCentreStatus, updateProcurementStage, createException, advanceCentreQueue, resetCentreAPI } from '../../services/api';
...
export default function OperatorDashboard({ centres, selectedCentreId = 'centre-1', onDataChanged }) {
  ...
  // Exception & Reset modal states
  const [showExModal, setShowExModal] = useState(false);
  const [showResetModal, setShowResetModal] = useState(false);
  const [resettingCentre, setResettingCentre] = useState(false);
  ...
  const handleCallNextToken = async () => {
    ...
    try {
      ...
      console.error('Error calling next token:', e);
    } finally {
      setAdvancingQueue(false);
    }
  };

  const handleResetCentreConfirm = async () => {
    setResettingCentre(true);
    try {
      const res = await resetCentreAPI(centre.id);
      if (res.success) {
        setShowResetModal(false);
        if (onDataChanged) onDataChanged();
      }
    } catch (e) {
      console.error('Error resetting centre:', e);
    } finally {
      setResettingCentre(false);
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
            <button
              ...
              style={{ background: '#22c55e', color: '#0f172a', border: 'none', padding: '0.45rem 0.9rem', borderRadius: '8px', fontWeight: 800, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
            >
              <Play size={15} /> {advancingQueue ? 'Calling...' : '▶ Call Next Token'}
            </button>
            <button
              onClick={() => setShowResetModal(true)}
              style={{ background: '#f59e0b', color: '#0f172a', border: 'none', padding: '0.45rem 0.9rem', borderRadius: '8px', fontWeight: 800, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}
            >
              <RotateCcw size={15} /> RESET CENTRE
            </button>
      ...
      {/* RESET CENTRE CONFIRMATION MODAL (Requirement 20) */}
      {showResetModal && (
        <div className="modal-overlay">
          <div className="modal-content" style={{ maxWidth: '480px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ margin: 0, color: '#b45309', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <RotateCcw size={20} /> Reset {centre.name}?
              </h3>
              <button onClick={() => setShowResetModal(false)} style={{ border: 'none', background: '#f1f5f9', borderRadius: '50%', width: '28px', height: '28px', cursor: 'pointer' }}>✕</button>
            </div>

            <div style={{ fontSize: '0.85rem', color: '#334155', lineHeight: 1.5, marginBottom: '1.2rem' }}>
              <p style={{ fontWeight: 700, margin: '0 0 0.5rem 0' }}>This action will reset today's operational data for this centre:</p>
              <ul style={{ margin: '0 0 1rem 1.2rem', padding: 0 }}>
                <li>Today's active Queue & Token sequence</li>
                <li>Current processing farmer status</li>
                <li>Temporary centre load & daily booked capacity</li>
                <li>Demo procurement activity for this yard</li>
              </ul>
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '0.65rem 0.85rem', borderRadius: '8px', color: '#14532d', fontSize: '0.8rem', fontWeight: 600 }}>
                🔒 <strong>Permanent Data Protection:</strong> Farmer accounts, registered land parcels, crop records, and historical payment records will <strong>NOT</strong> be deleted.
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
              ...
              <button
                onClick={handleResetCentreConfirm}
                disabled={resettingCentre}
                style={{ background: '#d97706', color: 'white', border: 'none', padding: '0.55rem 1.2rem', borderRadius: '8px', fontWeight: 800, fontSize: '0.85rem', cursor: 'pointer' }}
              >
                {resettingCentre ? 'Resetting...' : 'Reset Centre'}
              </button>
            </div>
          </div>
        </div>
