...
// ── Local Date & Time Utilities (PC/Mobile system clock) ─
...
export const fetchSlots = async(centreId, dateStr) => {
    // Always use local PC/Mobile date if none provided — ensures slot grid matches client clock
...
export const recordWeighmentAPI = async(centreId, appointmentId, actualWeightKg) => {
    const res = await fetch(`${API_BASE}/centres/${centreId}/queue/weighment`, {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ appointment_id: appointmentId, actual_weight_kg: actualWeightKg })
    });
    return res.json();
};
...
export const recordQualityAPI = async(centreId, appointmentId, grade, moisture) => {
    const res = await fetch(`${API_BASE}/centres/${centreId}/queue/quality`, {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ appointment_id: appointmentId, grade, moisture })
    });
    return res.json();
};
...
// Update crop MSP rate (Officer control panel action)
export const updateCropPriceAPI = async({ product_id, new_price_per_kg, reason, officer_name }) => {
    const res = await fetch(`${API_BASE}/products/update-price`, {
        method: 'POST',
        headers: authHeaders(),
        body: JSON.stringify({ product_id, new_price_per_kg, reason, officer_name })
    });
    return res.json();
};

// ── Local Date & Time Utilities (exported for use in components) ─
// Returns formatted local date+time label for PC/Mobile display
export const getLocalDateTimeLabel = () => {
    const now = new Date();
...
export const fetchLandParcels = async(farmerId) => {
    if (!farmerId) return { success: true, parcels: [] };
...
export const fetchFarmerCrops = async(farmerId) => {
    if (!farmerId) return { success: true, crops: [] };
